...
import verifyOldPassword from "../../api/authentication/verify-old-password";
...
function Profile() {
    ...
    const handleName = (event) => setUser(values => ({...values, name: event.target.value}));
    const handleBirthday = (event) => setUser(values => ({...values, birthday: event.target.value}));
    const handleLanguage = (event) => setUser(values => ({...values, language: event.value}));
    const handleEmail = (event) => setUser(values => ({...values, email: event.target.value}));
    const handleTheme = (event) => setUser(values => ({...values, theme: event.target.value}));
    const handleSurname = (event) => setUser(values => ({...values, surname: event.target.value}));
    const handleUsername = (event) => setUser(values => ({...values, username: event.target.value}));
    const handleFavCategories = (event) => setUser(values => ({...values, favoriteCategories: event.value}));
    ...
    useEffect(() => {
        ...
        fetchLanguages().then(result => setLanguages(result));
    }, []);
    ...
    const saveProfile = async () => {
        ...
        const response = await saveProfileInfo(({...user, isEnabled: user.enabled, birthday}), avatar);
    ...
    const savePassword = async () => {
        ...
        const response = await changePassword(CHANGE_PASSWORD_URL, {oldPassword, password});
        if (response) {
